...
const sortByChildren = (charArray) => {
  ...
  charArray.sort((a,b) => { return a.children.length - b.children.length;});
  return charArray;
};
...
const updateNumbers = (obj) => {
  ...
  //Object.entries(obj) = [ [Grace,222-303],[,],........]
  //item.join(': ') // [Grace,222-303] => 'Grace: 222-303'
  let arr = [];
...
const getHouses = (arr) => {
  let houses = [];
  // characters [] --> foreach {} --> object.value({}) --> arr [, , , ,house]
  //--> arr[3] --> push
  characters.forEach(item => {
    houses.push(Object.values(item)[3]);
  });
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  let flag = 0;
  arr.forEach(item => {
    if (Object.values(item)[0] === character)
    {
  ...
  return flag; //you cant put return inside forEach it will stop function
