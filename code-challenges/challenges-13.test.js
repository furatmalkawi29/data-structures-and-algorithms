...
const findHappiness = (arr) => {
  let happinessArr = arr.filter(item=>
  {
...
const standardizePhoneNumbers = (arr) => {
  let standArr = arr.map (item =>
  {
    let strArr = [];
    strArr.push(item.slice(1,4));
    strArr.push(item.slice(6,9));
    strArr.push(item.slice(10,14));
...
const onlyOddChars = (str) => {
  ...
  let oddCharStr = str.split('').filter((item,index) =>{
    return index%2;
  }).join('');
...
const allHappy = (arr) => {
  let happyResult = arr.filter(item =>
  {
    return item.includes(':)');
  });

  return happyResult.length === arr.length;
};
