...
const addOne = (arr) => {
  ...
  let arr2 =[]; // let arr2; --> arr2 will be undefined later
...
const addExclamation = (arr) => {
  // arr called input array
  let arr2 =[]; // called local array
  arr.forEach(item => {
    arr2.push(`${item}!`);
...
const addNumbers = (num, arr, times, callback) => {
  ...
  let arr2 = [];
  for (let i =0; i<times;i++) {callback(arr2,num);}
...
const createList = (availableItems) => {
  ...
  availableItems.forEach( item => {
    if (item.available === true ) {list.push(item.name);}
  });
