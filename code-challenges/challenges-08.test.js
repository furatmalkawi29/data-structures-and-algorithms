...
function returnTen(str){
  let arr = str.split(''); // 'hello world' --> ['h', 'e', 'l', 'l', ....]
  let startIndex = arr.length - 10 ; // idx = 1
  let arrResult = arr.splice(startIndex,10); // ['e' ,'l' ,'l' ,.....]
...
const typeNum = (arr) => {
  let arr2 = arr.filter(element => {
    //NaN : Not a Number
    //isNaN(value) --> if value isnt number : return true;
...
const containsAnd = (arr) => {
  ...
  let arr2 = arr.filter(element => {
    let regEx = /and/g; // had to put regex inside loop so the code works/ why?
    let flag = regEx.test(element);
...
const filterStringsWithVowels = (arr) => {
  let arr2 = arr.filter(element => {
    // had to put regex inside so the loop works/ why?
    let regEx = /[ A, E, I, O, U]/gi; //spaces are important --> [ f, s, t]
