...
------------------------------------------------------------------------------------------------ */
function countArr (integer, arr)
{
  let intCount = arr.reduce((count,num)=>
  {
    if(integer === num)
      count++;
    return count;
...
const count = (target, input) => {
  let myCount = input.reduce((sum,element)=>
  {
    return sum = sum + countArr(target,element);
  },0);
  return myCount;
};
...
const totalSum = (input) => {
  let totalSum = input.reduce((sum,item)=>
  {
    return sum = sum + sumArr(item);
  },0);
...
//input: [ [0,2,5,4], [2,4,10], [] ]
function filterInput (matrix)
{
  let filterMatrix = matrix.map(arr => {
    let filterResult = arr.filter(num=> {
...
const divisibleByFiveTwoToThePower = (input) => {
  let filterMatrix= filterInput(input);

  let powerMatrix = filterMatrix.map(arr=>{
    let powerArr = arr.map(num=>{
...
let findShortest = (data) => {
  let shortest = data.reduce((min,item)=>{
    if (Number(item.height)<Number(min.height))
    {
      min = item;
    }
    return min;
