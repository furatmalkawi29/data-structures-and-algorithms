...
const sortStarWarsCharacters = (starWarsArr) => {
  ...
  starWarsArr.sort((a,b) => { return b.height - a.height;});
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach( (item) => {
    ...
    //search for second space (space before ingredient name)
    // start search after firstSpace by one index
    let secondSpace = item.indexOf(' ',firstSpace +1);

    // slice the rest of string after the secondSpace by one index
    result.push(item.slice(secondSpace+1));
  });
  return result;
};
