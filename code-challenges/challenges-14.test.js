...
const screenForNames = (arr) => {
  let regex = [/^Mr. \w/g, /^Mrs. \w/g, /^Ms. \w/g, /^Dr. \w/g];
  let resultArr =[];
  arr.forEach(item=>{
    regex.forEach(exp=> {
      if(exp.test(item)){
...
const toTitleCase = (arr) => {
  let resultArr = arr.map(item => {
    ...
    let letter = item.charAt(0).toUpperCase();
    return item.replace(item.charAt(0), letter);
  });
  return resultArr;
};
...
let biggerThanLuke = (arr) => {
  ...
  let lukeMass = arr.filter(item => item.name==='Luke Skywalker');

  let resultArr = arr.filter(item =>Number(item.mass)>Number(lukeMass[0].mass));

  let resultString = resultArr.map(item=> item.name).join(' - ');
...
function checkWin (inX1,inY1,inX2,inY2,inX3,inY3,arr){
  let checkEmpty = arr[inX1][inY1]==='' && arr[inX2][inY2] ==='' && arr[inX3][inY3] ==='';

  return arr[inX1][inY1] === arr[inX2][inY2] && arr[inX1][inY1] === arr[inX3][inY3] && arr[inX2][inY2] === arr[inX3][inY3] && !checkEmpty;
}

const detectTicTacToeWin = (board) => {
  let checkArr =[];
  board.forEach((arr,index)=>{
    checkArr.push(checkWin(index,0,index,1,index,2,board));
    checkArr.push(checkWin(0,index,1,index,2,index,board));
  });
  checkArr.push(checkWin(0,0,1,1,2,2,board));
