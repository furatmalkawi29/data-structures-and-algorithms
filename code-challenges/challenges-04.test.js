...
const isNum = (input) => {
  let strInput = input.toString();//regex only works with string
  let regex = /\d/g ;
...
const isCapitalized = (str) => {
  //---- zero matches --> math() returns: null --> test wants : []
  ...
  // null : is falsy
  // sentence eqivilent to:
  //if (arr) return arr; else return [];
  return arr? arr : [] ;
};
...
const citiesAtoJ = (arr) => {
  ...
  let regEx = /^[A-J]/gi ; // regEx should have no spaces between delimiters / / !
