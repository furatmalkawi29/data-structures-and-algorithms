...
function transformToLis(obj){
  ...
  //1) use for..in : itirate on --> Object properties
  for (let element in obj) // property = "name" //property = "age"
  {
    arrList.push(`<li>${element}: ${obj[element]}</li>`);
  }
...
const addValues = (arr) => {
  let result = arr.reduce((total,element)=> {
    total = total + element;
    return total;
...
const returnNames = (arr) => {
  let arr2 = arr.reduce( (resultArr,element) => {
    resultArr = resultArr + '.' + element.name;
    return resultArr;
  }, '');
...
const reversedString = (str) => {
  ...
  let reversed = arr.reduce((total,element)=>{
    total = element + total;
    return total;
  },'');
  return reversed;
